...
- (void)createUI{
    ...
    _mainScrollView.contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, IPhone4_5_6_6P(568, 590, 590, 590));
    [self addSubview:_mainScrollView];
    
    self.imageView1 = [[UIImageView alloc]initWithFrame:CGRectMake(IPhone4_5_6_6P(MSW / 2 - 50, MSW / 2 - 50, MSW / 2 - 60, MSW / 2 - 60), 30, IPhone4_5_6_6P(100, 100, 120, 120), IPhone4_5_6_6P(100, 100, 120, 120))];
    _imageView1.backgroundColor=[UIColor clearColor];
    
    _imageView1.image=[UIImage imageNamed:@"pic_empty_me_win"];
    [_mainScrollView addSubview:_imageView1];
    
    _titleLabel1=[[UILabel alloc]initWithFrame:CGRectMake(MSW/2-110, _imageView1.frame.origin.y+130, 220, 30)];
    _titleLabel1.text=@"你还没有中奖记录";
    _titleLabel1.font=[UIFont systemFontOfSize:BigFont];
    _titleLabel1.textAlignment=NSTextAlignmentCenter;
    [_mainScrollView addSubview:_titleLabel1];
    
    _textLabel1=[[UILabel alloc]initWithFrame:CGRectMake(MSW/2-100, _imageView1.frame.origin.y+170,200, 30)];
    _textLabel1.text=@"继续加油夺宝啊！";
    _textLabel1.textColor=[UIColor grayColor];
    _textLabel1.font=[UIFont systemFontOfSize:MiddleFont];
    _textLabel1.textAlignment=NSTextAlignmentCenter;
    
    self.backgroundColor=[UIColor whiteColor];
    [_mainScrollView addSubview:_textLabel1];

    _btgoto=[[UIButton alloc]initWithFrame:CGRectMake(MSW/2-100, _imageView1.frame.origin.y+200,200, 40)];
    ...
    _btgoto.layer.cornerRadius = 5;
    _btgoto.backgroundColor=RGBCOLOR(29, 146, 255);
    [_mainScrollView addSubview:_btgoto];
    
    _likeView = [[UIView alloc] initWithFrame:CGRectMake(0, MSH - 40-200 - 110 , MSW, 40)];
    ...
    _scrolike=[[UIScrollView alloc]initWithFrame:CGRectMake(0, _likeView.bottom, MSW, 200)];
...
-(void)Lasetesannaction:(UIButton *)sender{
    ...
    if (_type==1) {
        NSNotification *wushuju =[NSNotification notificationWithName:@"wushuju" object:nil userInfo:dict];
        
        [[NSNotificationCenter defaultCenter] postNotification:wushuju];
    }else if (_type==2){
        NSNotification *gerentz =[NSNotification notificationWithName:@"gerentz" object:nil userInfo:dict];
        
        [[NSNotificationCenter defaultCenter] postNotification:gerentz];
...
-(void)getuserlike{
    ...
    [MDYAFHelp AFPostHost:APPHost bindPath:Userlike postParam:nil getParam:nil success:^(AFHTTPRequestOperation *operation, NSDictionary *responseDic) {
        DebugLog(@"栏目：%@－－－－－－%@",responseDic,responseDic[@"msg"]);
...
-(void)initData{
    ...
    for (int i = 0; i < _likeArray.count; i ++)
    {
        ...
        UIImageView*imggood=[[UIImageView alloc]initWithFrame:CGRectMake(20, 15, MSW/3-40 , MSW/3-40)];
        ...
        UILabel*lblook=[[UILabel alloc]initWithFrame:CGRectMake(10, imggood.frame.origin.y+imggood.frame.size.height+5, MSW/3-20, 40)];
    ...
    if (_type==1||_type==3) {
        ...
        _lbline2.hidden=YES;
        _scrolike.hidden=YES;
        _mainScrollView.contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, 300);
  ...
  for (int i = 0; i < _goodsarray.count; i ++)
    {
        ...
        UILabel*lblook=[[UILabel alloc]initWithFrame:CGRectMake(10, imggood.frame.origin.y+imggood.frame.size.height+5, MSW/3-20, 30)];
        lblook.numberOfLines=2;
        lblook.text=[_goodsarray objectAtIndex:i];
        lblook.textAlignment=UITextAlignmentCenter;
        lblook.font=[UIFont systemFontOfSize:BigFont];
        lblook.textColor=[UIColor redColor];
        [LatestView addSubview:lblook];
        
        UILabel*lbline2=[[UILabel alloc]initWithFrame:CGRectMake(0,MSW/3+28 , MSW/3, 2)];
        lbline2.backgroundColor=[UIColor groupTableViewBackgroundColor];
        [LatestView addSubview:lbline2];
        UIButton*btdetai=[[UIButton alloc]initWithFrame:CGRectMake(0, 30, MSW/3, MSW/3)];
        btdetai.backgroundColor=[UIColor clearColor];
        btdetai.tag=i;
        [btdetai addTarget:self action:@selector(Lasetesannaction:) forControlEvents:UIControlEventTouchUpInside];
