...
- (void)dismissWithStatus2:(NSString*)string error:(BOOL)error;
- (void)dismissWithStatus:(NSString*)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds;
- (void)dismissWithStatus2:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds;
@end
...
+ (void)showSuccessWithStatus2:(NSString *)string {
    [SVProgressHUD showSuccessWithStatus2:string duration:1];
}
+ (void)showSuccessWithStatus:(NSString *)string duration:(NSTimeInterval)duration {
    [SVProgressHUD show];
    [SVProgressHUD dismissWithSuccess:string afterDelay:duration];
}
+ (void)showSuccessWithStatus2:(NSString *)string duration:(NSTimeInterval)duration {
    [SVProgressHUD show];
    [SVProgressHUD dismissWithSuccess2:string afterDelay:duration];
}
...
+ (void)dismissWithSuccess2:(NSString *)successString afterDelay:(NSTimeInterval)seconds {
    [[SVProgressHUD sharedView] dismissWithStatus2:successString error:NO afterDelay:seconds];
}
...
- (void)dismissWithStatus2:(NSString*)string error:(BOOL)error {
    [self dismissWithStatus2:string error:error afterDelay:0.9];
}
- (void)dismissWithStatus2:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        if(self.alpha != 1)
...
- (UIView *)hudView {
    if(!hudView) {
        ...
        hudView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.65];
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        ...
        stringLabel.font = [UIFont boldSystemFontOfSize:BigFont];
