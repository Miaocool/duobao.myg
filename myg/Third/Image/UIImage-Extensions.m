...
{
	return [self imageRotatedByDegrees:RadiansToDegreesA(radians)];
}
...
+ (UIImage *)getImageFormImageName:(NSString *)imageName
{
    ...
    CGFloat widthScale = img.size.width / 320;
    CGFloat heightScale = img.size.height / 120;

    CGRect partRect;
    if (widthScale > heightScale) {
        CGFloat width = img.size.width * 120.0 / img.size.height;
        partRect = CGRectMake((img.size.width - width) / 2.0, 0, width, img.size.height);
    }else{
        CGFloat height = img.size.height * 320.0 / img.size.width;
        partRect = CGRectMake(0, (img.size.height - height) / 2.0, img.size.width, height);
    }
