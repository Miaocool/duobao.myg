                     ...
                     picFileName: (NSString *)picFileName;  // IN
{
    ...
    for(int i=0;i<[keys count];i++)
    {
        ...
        DebugLog(@"添加字段的值==%@",[postParems objectForKey:key]);
    }
...
//        NSString *path = EncodeFormDic(logoDic, @"img");
//        DebugLog(@"!!!!!!!!%@",path);
        return path;
    }
...
+ (NSString *)saveImage:(UIImage *)tempImage WithName:(NSString *)imageName{
    ...
    DebugLog(@"===fullPathToFile===%@",fullPathToFile);
    DebugLog(@"===FileName===%@",[nameAry objectAtIndex:[nameAry count]-1]);
